...
fn nmodl_param_block(coll: &Collapsed) -> Result<String> {
    if coll.parameters.is_empty() { return Ok(String::new()); }
...
fn ion_species(coll: &Collapsed) -> String { coll.attributes.get("species").cloned().flatten().unwrap_or_else(String::new) }
