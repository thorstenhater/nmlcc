...
impl XML for Simulation {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "DataDisplay" => {
                    body.push(SimulationBody::DataDisplay(DataDisplay::from_node(&child)))
                }
                "Record" => body.push(SimulationBody::Record(Record::from_node(&child))),
                "EventRecord" => {
                    body.push(SimulationBody::EventRecord(EventRecord::from_node(&child)))
                }
                "Run" => body.push(SimulationBody::Run(Run::from_node(&child))),
                "DataWriter" => {
                    body.push(SimulationBody::DataWriter(DataWriter::from_node(&child)))
                }
                "EventWriter" => {
                    body.push(SimulationBody::EventWriter(EventWriter::from_node(&child)))
                }
    ...
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                ...
                "Parameter" => {
                    body.push(ComponentTypeBody::Parameter(Parameter::from_node(&child)))
                }
                "DerivedParameter" => body.push(ComponentTypeBody::DerivedParameter(
                    DerivedParameter::from_node(&child),
                )),
                "IndexParameter" => body.push(ComponentTypeBody::IndexParameter(
                    IndexParameter::from_node(&child),
                )),
                ...
                "ComponentReference" => body.push(ComponentTypeBody::ComponentReference(
                    ComponentReference::from_node(&child),
                )),
                "Attachments" => body.push(ComponentTypeBody::Attachments(Attachments::from_node(
                    &child,
                ))),
                "EventPort" => {
                    body.push(ComponentTypeBody::EventPort(EventPort::from_node(&child)))
                }
                "Exposure" => body.push(ComponentTypeBody::Exposure(Exposure::from_node(&child))),
                "Requirement" => body.push(ComponentTypeBody::Requirement(Requirement::from_node(
                    &child,
                ))),
                "ComponentRequirement" => body.push(ComponentTypeBody::ComponentRequirement(
                    ComponentRequirement::from_node(&child),
                )),
                "InstanceRequirement" => body.push(ComponentTypeBody::InstanceRequirement(
                    InstanceRequirement::from_node(&child),
                )),
                ...
                "Structure" => {
                    body.push(ComponentTypeBody::Structure(Structure::from_node(&child)))
                }
                "Simulation" => {
                    body.push(ComponentTypeBody::Simulation(Simulation::from_node(&child)))
                }
...
impl XML for Regime {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "TimeDerivative" => body.push(RegimeBody::TimeDerivative(
                    TimeDerivative::from_node(&child),
                )),
...
impl XML for Dynamics {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "StateVariable" => body.push(DynamicsBody::StateVariable(
                    StateVariable::from_node(&child),
                )),
                "DerivedVariable" => body.push(DynamicsBody::DerivedVariable(
                    DerivedVariable::from_node(&child),
                )),
                "ConditionalDerivedVariable" => {
                    body.push(DynamicsBody::ConditionalDerivedVariable(
                        ConditionalDerivedVariable::from_node(&child),
                    ))
                }
                "TimeDerivative" => body.push(DynamicsBody::TimeDerivative(
                    TimeDerivative::from_node(&child),
                )),
                ...
                "OnCondition" => {
                    body.push(DynamicsBody::OnCondition(OnCondition::from_node(&child)))
                }
                "Regime" => body.push(DynamicsBody::Regime(Regime::from_node(&child))),
                "KineticScheme" => body.push(DynamicsBody::KineticScheme(
                    KineticScheme::from_node(&child),
                )),
...
impl XML for KineticScheme {
    fn from_node(node: &Node) -> Self {
        ...
        let stateVariable = node
            .attribute("stateVariable")
            .map(|s| s.to_string())
            .unwrap();
        ...
        let forwardRate = node
            .attribute("forwardRate")
            .map(|s| s.to_string())
            .unwrap();
        let reverseRate = node
            .attribute("reverseRate")
            .map(|s| s.to_string())
            .unwrap();
        KineticScheme {
            name,
...
impl XML for OnEntry {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "StateAssignment" => body.push(OnEntryBody::StateAssignment(
                    StateAssignment::from_node(&child),
                )),
...
impl XML for OnCondition {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "StateAssignment" => body.push(OnConditionBody::StateAssignment(
                    StateAssignment::from_node(&child),
                )),
                "EventOut" => body.push(OnConditionBody::EventOut(EventOut::from_node(&child))),
                "Transition" => {
                    body.push(OnConditionBody::Transition(Transition::from_node(&child)))
                }
    ...
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                ...
                "ComponentType" => {
                    body.push(LemsBody::ComponentType(ComponentType::from_node(&child)))
                }
...
impl XML for OnEvent {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "StateAssignment" => body.push(OnEventBody::StateAssignment(
                    StateAssignment::from_node(&child),
                )),
...
impl XML for ConditionalDerivedVariable {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "Case" => body.push(ConditionalDerivedVariableBody::Case(Case::from_node(
                    &child,
                ))),
                t => panic!(
                    "Unexpected tag {} in body of ConditionalDerivedVariable.",
                    t
                ),
            };
        }
...
impl XML for Structure {
    fn from_node(node: &Node) -> Self {
        ...
        for child in node.children() {
            ...
            match child.tag_name().name() {
                "ChildInstance" => body.push(StructureBody::ChildInstance(
                    ChildInstance::from_node(&child),
                )),
                "MultiInstantiate" => body.push(StructureBody::MultiInstantiate(
                    MultiInstantiate::from_node(&child),
                )),
                ...
                "EventConnection" => body.push(StructureBody::EventConnection(
                    EventConnection::from_node(&child),
                )),
