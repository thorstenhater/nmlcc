...
fn simple_type(node: &Node) -> Result<(String, Kind)> {
    let name = node
        .attribute("name")
        .ok_or_else(|| String::from("Unnamed simple type."))?;
...
fn element(node: &Node) -> Result<Element> {
    let name = node
        .attribute("name")
        .ok_or_else(|| String::from("Unnamed element."))?;
    let typ = node
        .attribute("type")
        .ok_or_else(|| String::from("Untyped element."))?;
...
fn group_def(node: &Node) -> Result<(String, Body)> {
    let name = node
        .attribute("name")
        .ok_or_else(|| String::from("Unnamed group"))?;
    for child in node.children() {
        if child.is_comment() || child.is_text() {
            continue;
        }
...
fn complex_type(node: &Node) -> Result<(String, Type)> {
    let name = node
        .attribute("name")
        .ok_or_else(|| String::from("Unnamed type"))?;
    let mut members = Vec::new();
    let mut body = Body::Empty;
    let mut base = None;
    for child in node.children() {
        ...
        match child.tag_name().name() {
            ...
            "complexContent" => {
                if base.is_some() {
                    return Err(String::from("ComplexType must have at most one base."));
                }
...
fn dealias(tys: &Map<String, Type>, alias: &Map<String, Kind>) -> Map<String, Type> {
    ...
    for (n, ty) in tys {
        ...
        if let Body::Alt(es) = t.body {
            ...
            for e in es {
                let x = match e {
                    Element::Element(n, Kind::Class(c)) if alias.contains_key(&c) => {
                        Element::Element(n, alias.get(&c).unwrap().clone())
                    }
                    a => a.clone(),
                };
                rs.push(x);
...
fn emit_src(state: &Schema) -> Result<String> {
    ...
    let mut lines = vec![
        String::from("// This file is @generated by schema. Do not edit!"),
        String::from("#![cfg_attr(rustfmt, rustfmt_skip)]"),
        String::from("#![allow(non_camel_case_types, non_snake_case, unused_variables)]"),
        String::from("#![allow(clippy::many_single_char_names, clippy::large_enum_variant, clippy::derive_partial_eq_without_eq)]"),
        String::from(""),
    ...
    for (_, t) in data {
        ...
        for m in &t.members {
            ...
            let unwrap = if let Count::Opt(None) = &m.count {
                ""
            } else {
                ".unwrap()"
            };
            ...
            let kind = if let Count::Opt(None) = &m.count {
                format!("Option<{}>", m.data.to_rs())
            } else {
                m.data.to_rs()
            };
        ...
        match t.body {
            Body::Alt(ref xs) => {
                let mut lp = vec![
                    String::from("        let mut body = Vec::new();"),
                    String::from("        for child in node.children() {"),
                    String::from("            if child.is_comment() || child.is_text() {"),
...
fn main() -> Result<()> {
    ...
    out.write(emit_src(&data)?.as_bytes())
        .map_err(|e| e.to_string())?;

    let nm = format!(
        "{}/ext/LEMS/Schemas/LEMS/LEMS_v0.7.6.xsd",
        cwd.to_string_lossy()
    );
    let xml = std::fs::read_to_string(nm).map_err(|e| e.to_string())?;
    let data = schema(&xml)?;
